...
const APDesignationsList = () => {
  ...
  const getData = async () => {
    try {
      ...
      const response = await axios.post(
        BaseApi + "/admin/designation/index",
  ...
  const handleActivate = async (slug) => {
    try {
      ...
      if (confirmationResult.isConfirmed) {
        // setLoading(true);
        const response = await axios.post(
          BaseApi + `/admin/designation/admin_activate/${slug}`,
          null, // Pass null as the request body if not required
          {
            headers: {
              "Content-Type": "application/json",
  ...
  const handleDeactivate = async (slug) => {
    try {
      ...
      if (confirmationResult.isConfirmed) {
        // setLoading(true);
        const response = await axios.post(
          BaseApi + `/admin/designation/admin_deactivate/${slug}`,
          null, // Pass null as the request body if not required
          {
            headers: {
              "Content-Type": "application/json",
  ...
  const handleDelete = async (slug) => {
    try {
      ...
      if (confirmationResult.isConfirmed) {
        // setLoading(true);
        const response = await axios.post(
          BaseApi + `/admin/designation/admin_delete/${slug}`,
          null, // Pass null as the request body if not required
          {
            headers: {
              "Content-Type": "application/json",
  ...
  const handleMultipleDeactivate = async () => {
    try {
      const confirmationResult = await Swal.fire({
        title: "Deactivate Designations?",
        text: "Do you want to Deactivate these Designations?",
        icon: "question",
        showCancelButton: true,
        confirmButtonText: "Yes",
        cancelButtonText: "No",
      ...
      if (confirmationResult.isConfirmed) {
        ...
        if (response.data.status === 200) {
          Swal.fire({
            title: "Designations Deactivated successfully!",
            icon: "success",
            confirmButtonText: "Close",
          });
        } else {
      ...
      Swal.fire({
        title: "Failed. Please try after some time!",
        text: "Could not Deactivate Designations",
        icon: "error",
        confirmButtonText: "Close",
      });
      console.log("Couldn't deactivate the record!", error.message);
  ...
  const handleMultipleActivate = async () => {
    try {
      const confirmationResult = await Swal.fire({
        title: "Activate Designations?",
        text: "Do you want to Activate these Designations?",
        icon: "question",
        showCancelButton: true,
        confirmButtonText: "Yes",
        cancelButtonText: "No",
      ...
      if (confirmationResult.isConfirmed) {
        ...
        if (response.data.status === 200) {
          Swal.fire({
            title: "Designations Activated successfully!",
            icon: "success",
            confirmButtonText: "Close",
          });
        } else {
      ...
      Swal.fire({
        title: "Failed. Please try after some time!",
        text: "Could not Activate Designations",
        icon: "error",
        confirmButtonText: "Close",
      });
      console.log("Couldn't activate the record!", error.message);
  ...
  const handleMultipleDelete = async () => {
    try {
      const confirmationResult = await Swal.fire({
        ...
        text: "Do you want to Delete these Designations?",
        icon: "question",
        showCancelButton: true,
        confirmButtonText: "Yes",
        cancelButtonText: "No",
      ...
      if (confirmationResult.isConfirmed) {
        ...
        if (response.data.status === 200) {
          Swal.fire({
            title: "Designations Deleted successfully!",
            icon: "success",
            confirmButtonText: "Close",
          });
        } else {
      ...
      Swal.fire({
        title: "Failed. Please try after some time!",
        text: "Could not Delete Designations",
        icon: "error",
        confirmButtonText: "Close",
      });
      console.log("Couldn't Delete the record!", error.message);
          ...
          <>
            <div className="site-min-height">
              ...
              <div className="ManageSubAdminHeader">
                <h2 className="">Designations List</h2>

                {(userAccess[1]?.Add === 1 || adminID === "1") && (
                  <>
                    <button
                      className="btn navButton1 APMSbutton"
                      onClick={() =>
                        navigate("/admin/designations/adddesignations")
                      }
                    >
                      Add Designations
                    </button>
                ...
                <>
                  <div className="manageSubadminPart1">
                    <form>
                      <div className="manageSubadminPart1Sec1">
                        <h4>Search Designation by Name</h4>
                                  ...
                                  {(userAccess[1]?.Edit === 1 ||
                                    ...
                                    <>
                                      <button
                                        ...
                                        onClick={() =>
                                          navigate(
                                            `/admin/designations/editdesignation/${i.slug}`
                                          )
                                        }
                                      >
                                        <Tooltip title="Edit">
                  ...
                  <div className="manageSubadminpart3">
                    <div className="functionalityButton">
                      ...
                      {(userAccess[1]?.Delete === 1 || adminID === 1) && (
                        <>
                          <button
                            type="button"
...
export default APDesignationsList;
