...
class Plan extends Model
{
    ...
    public function checkPlanFeature($userId=null, $fid=null){
        ...
        if($myPlan == 0){
            ...
            $pdata['status'] = 0;
            $pdata['message'] = 'You do not have plan, please purchase suitable plan to access website functionality.';
            return $pdata;
        }


        $myPlan = User_plan::select(['user_plans.id','user_plans.features_ids','user_plans.fvalues','user_plans.start_date','user_plans.end_date','user_plans.is_expire'])
        ->join('plans', 'plans.id', '=', 'user_plans.plan_id')
        ->where('user_id',$userId)
        // ->where('plans.plan_type', 1)
        ->where('user_plans.is_expire' , 0)
        ->orderBy('user_plans.invoice_no','DESC')
        ->first();
        ...
        switch ($fid) {
            ...
            if(in_array(4, $featuresArray)){
                ...
                if($postJobCount >= $maxJobPost){
                   ...
                   $pdata['status'] = 1; 
                   $pdata['user_plan_id'] = $myPlan->plan_id;
                }
            }else{
    ...
    public function checkPlanFeatureNew($userId=null, $fid=null){
        $myPlan = $this->getcurrentplanEXPNew($userId);
        if($myPlan == 0){
            $pdata = array('status'=>0,'message'=>'');
            $pdata['status'] = 0;
            $pdata['message'] = 'You do not have plan, please purchase suitable plan to access website functionality.';
    ...
    public function getcurrentplanEXP($userId=null){
        ...
        ->join('plans', 'plans.id', '=', 'user_plans.plan_id')
        ->where('user_plans.start_date' , '<=' , $date)
        // ->where('plans.plan_type', 1)
    ...
    public function getcurrentplanEXPNew($userId=null){
        $date = date('Y-m-d');
    ...
    public function getcurrentplan($userId=null){
        ...
        $userPlan = User_plan::select(['user_plans.id','user_plans.features_ids','user_plans.fvalues','user_plans.start_date','user_plans.end_date','user_plans.is_expire','user_plans.is_expire','user_plans.invoice_no','user_plans.payment_id','user_plans.user_id','user_plans.plan_id','user_plans.amount','user_plans.type','user_plans.type_value','user_plans.created','user_plans.slug'])
        ->join('plans', 'plans.id', '=', 'user_plans.plan_id')
        ->where('user_id',$userId)
        ->where('is_expire',0)
        // ->where('plans.plan_type', 1)
    ...
    public function getcurrentPartnershipplan($userId=null){
        $date = date('Y-m-d');
    ...
    public function getPartnershiplanFeature($userId=null){
       
        $date = date('Y-m-d');
        $pdata = array('status'=>0);

        $myPlan = $this->getcurrentPartnershipplan($userId);
   
   
        if(!empty($myPlan)){
            $pdata = array('status'=>1);
            $features = $myPlan->features_ids;
            $user_plan_id = $myPlan->plan_id;
            $featuresArray = explode(',', $features);
            $fvalues = json_decode($myPlan->fvalues, true);
            $sdate = $myPlan->start_date;
            $edate = $myPlan->end_date;
            ...
            if(in_array(3, $featuresArray)){
                 $pdata['searchCandidate'] ='Yes';
            }else{
                
                $pdata['searchCandidate'] ='No';
    ...
    public function getPlanFeature($userId=null){
        ...
        if(!empty($myPlan)){
            ...
            if(in_array(4, $featuresArray)){
                ...
                $pdata['appliedCount'] =$appliedCount;
             //   $pdata['userId'] =$userId;
             //   $pdata['user_plan_id'] =$user_plan_id;
            //     print_r($userId);
            // print_r('******');
            // print_r($user_plan_id);exit;
